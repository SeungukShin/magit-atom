...
export default class Git {
	...
	async getConfig(key: String): Promise<String> {
		return this.execute('git', ['config', '--get-all', key]);
	}
...
//		return this.execute('git', ['rev-parse', '--abbrev-ref', 'HEAD']);
	...
	async getUpstreamMerge(branch: String = null): Promise<String> {
		if (!branch) {
			branch = await this.getCurrentBranch();
		}
	...
	async getUpstreamRebase(branch: String = null): Promise<String> {
		if (!branch) {
			branch = await this.getCurrentBranch();
		}
	...
	async getPushRemote(branch: String = null): Promise<String> {
		if (!branch) {
			branch = await this.getCurrentBranch();
		}
		const remote = this.getConfig(`branch.${branch}.pushRemote`);
		if (remote.len > 0) {
			return remote;
		}
		return this.getConfig('remote.pushDefault');
	}
