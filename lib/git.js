...
//		return this.execute('git', ['rev-parse', '--abbrev-ref', 'HEAD']);
	...
	rebaseApply(): Boolean {
		const file = path.join(this.cwd, '.git', 'rebase-apply');
		return fs.existsSync(file);
	}
	...
	getSeqHashes(file: String): String[] {
		const lines = fs.readFileSync(file, 'utf8');
		const results = [];
	...
	getSeqActions(file: String): {act: String, hash: String, subject: String}[] {
		const lines = fs.readFileSync(file, 'utf8');
		const results = [];
		for (const line of lines.split('\n')) {
			...
			results.push({
				act: line.substring(0, actEnd),
				hash: line.substring(actEnd + 1, hashEnd),
				subject: line.substring(hashEnd + 1)
			});
		}
		return results;
	}

	getRebaseMergeTodo(): {act: String, hash: String, subject: String}[] {
		const baseDir = path.join(this.cwd, '.git', 'rebase-merge');
		const file = path.join(baseDir, 'git-rebase-todo');
		return this.getSeqActions(file);
	}

	getRebaseMergeStop(): String[] {
		const baseDir = path.join(this.cwd, '.git', 'rebase-merge');
		const file = path.join(baseDir, 'stopped-sha');
		return this.getSeqHashes(file);
	}

	getRebaseMergeDone(): {act: String, hash: String, subject: String}[] {
		const baseDir = path.join(this.cwd, '.git', 'rebase-merge');
		const file = path.join(baseDir, 'done');
		return this.getSeqActions(file);
	}
