...
export default class Git {
	...
	async getDiffFile(args: String[], file: String): Promise<GitDiff> {
		let gitDiff = new GitDiff();
		const base = ['diff', '--ita-invisible-in-index', '--show-signature', '--no-prefix'];
		if (args.length > 0) {
		...
		return this.executeObject('git', args, gitDiff);
	}
	...
	// @return: unstaged diff for @file
	async getUnstagedDiffFile(file: String): Promise<GitDiff> {
		return this.getDiffFile([], file);
	}
	...
	async getStagedFiles(): Promise<String[]> {
		return this.executeLines('git', ['diff-index', '--name-only', '--cached', 'HEAD']);
	}

	// get staged diff for @file
	// @return: staged diff for @file
	async getStagedDiffFile(file: String): Promise<GitDiff> {
		return this.getDiffFile(['--cached'], file);
	}
