...
export default class MagitStatus {
	...
	async insertRebaseSeq() {
		...
		const lines = this.git.getRebaseMergeTodo();
		for (const line of lines) {
			...
			this.insertText(tokens[0], GIT_STATUS_COLORS['subject']);
			this.insertText(` ${tokens[1]}`, GIT_STATUS_COLORS['hash']);
			this.insertText(` ${tokens[2]}\n`, GIT_STATUS_COLORS['subject']);
		}
