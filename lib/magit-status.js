...
import { CompositeDisposable, TextEditor, TextBuffer, Point, Range, DisplayMarker } from 'atom';
import Log from './log';
...
class FoldInfo {
	...
	checkRange(line: Number): Boolean {
		if (line < this.range.start.row || line > this.range.end.row) {
			return false;
		}
		return true;
	}
	...
	toggle(line: Number, editor: TextEditor): Boolean {
		if (editor.isFoldedAtBufferRow(line)) {
			...
			if (!this.checkRange(line)) {
				return false;
			}

			// children
			for (const child of this.children) {
				if (child.toggle(line, editor)) {
					return true;
				}
			}

			// no match with any children
...
export default class MagitStatus {
	...
	constructor(cwd: String) {
		...
		this.subscriptions.add(atom.commands.add(this.editor.element, {
			...
			'magit-atom:toggle-fold': () => {
				...
				for (const foldInfo of this.foldInfos) {
					if (foldInfo.toggle(line, this.editor)) {
						return;
					}
				}
			}
	...
	async insertHeaders() {
		...
		this.foldInfos.push(new FoldInfo(new Range(startPoint, endPoint)));
	}
