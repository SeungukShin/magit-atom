...
export default class MagitStatus {
	...
	async insertSummary() {
		...
		const upstream = await this.git.getUpstreamBranch();
		...
		// Tag
		const tag = await this.git.getCurrTag();
		if (tag.tag.length > 0) {
			this.insertText('Tag:'.padEnd(10), GIT_STATUS_COLORS['head']);
			this.insertText(` ${tag.tag}`, GIT_STATUS_COLORS['tag']);
			if (tag.dist > 0) {
				this.insertText(' (', GIT_STATUS_COLORS['subject']);
				this.insertText(`${tag.dist}`, GIT_STATUS_COLORS['local']);
				this.insertText(')', GIT_STATUS_COLORS['subject']);
			}
			this.insertText('\n');
		}
