	...
	async insertRebaseSeq() {
		...
		if (this.git.rebaseMergeInProgress()) {
			...
			for (const todo of todos) {
				const hash = await this.git.getSymRev(todo.hash);
				this.insertText(todo.act, GIT_STATUS_COLORS['subject']);
				this.insertText(` ${hash}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${todo.subject}\n`, GIT_STATUS_COLORS['subject']);
			}

			const stop = this.git.getRebaseMergeStop();
			if (stop.length > 0) {
				const log = await this.git.getLog(stop);
			...
			for (const done of dones) {
				const hash = await this.git.getSymRev(done.hash);
				this.insertText('done', GIT_STATUS_COLORS['hash']);
				this.insertText(` ${hash}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${done.subject}\n`, GIT_STATUS_COLORS['subject']);
			}
		} else {
			const rewrittens = this.git.getRebaseApplyRewrittens();
			for (const rewritten of rewrittens) {
				const log = await this.git.getLog(rewritten.newItem);
				const hash = log.getHash();
				const subject = log.getSubject();
				const msg = (subject.length > 0) ? subject : '(no commit message)';
				this.insertText('pick', GIT_STATUS_COLORS['subject']);
				this.insertText(` ${hash}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${msg}\n`, GIT_STATUS_COLORS['subject']);
			}

			const original = this.git.getRebaseApplyOriginalCommit();
			if (original.length > 0) {
				const log = await this.git.getLog(original);
				const hash = log.getHash();
				const subject = log.getSubject();
				const msg = (subject.length > 0) ? subject : '(no commit message)';
				this.insertText(`stop ${hash}`, GIT_STATUS_COLORS['hash']);
		...
		const subject = log.getSubject();
		const msg = (subject.length > 0) ? subject : '(no commit message)';
		this.insertText('onto', GIT_STATUS_COLORS['remote']);
		this.insertText(` ${hash}`, GIT_STATUS_COLORS['hash']);
		this.insertText(` ${msg}\n`, GIT_STATUS_COLORS['subject']);
	}
