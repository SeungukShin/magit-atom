...
export default class MagitStatus {
	...
	async insertTagsHeader() {
		...
		if (currTag.tag === nextTag.tag) {
			nextTag.tag = '';
		}
		const bothTag = (currTag.tag.length > 0 && nextTag.tag.length > 0);
		if (currTag.tag.length <= 0 && nextTag.tag.length <= 0) {
			return;
		}
		const head = (bothTag) ? 'Tags:' : 'Tag:';
		this.insertText(head.padEnd(10), GIT_STATUS_COLORS['head']);
		if (currTag.tag.length > 0) {
			this.insertText(` ${currTag.tag}`, GIT_STATUS_COLORS['tag']);
			if (currTag.dist > 0) {
	...
	async insertMergeLog() {
		...
		const shortMergeHead = await this.getShortHash(mergeHead);
	...
	async insertRebaseSeq() {
		...
		if (this.git.rebaseMerge()) {
			const todos = this.git.getRebaseMergeTodo();
			for (const todo of todos) {
				this.insertText(todo.act, GIT_STATUS_COLORS['subject']);
				this.insertText(` ${todo.hash}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${todo.subject}\n`, GIT_STATUS_COLORS['subject']);
			}

			const stops = this.git.getRebaseMergeStop();
			for (const stop of stops) {
				const gitLog = await this.git.getLog(stop);
				const hash = gitLog.getHash();
				const subject = gitLog.getSubject();
				const msg = (subject.length > 0) ? subject : '(no commit message)';
				this.insertText(`stop ${hash}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${msg}\n`, GIT_STATUS_COLORS['subject']);
			}

			const dones = this.git.getRebaseMergeDone();
			for (const done of dones) {
				this.insertText('done', GIT_STATUS_COLORS['hash']);
				this.insertText(` ${await this.git.getShortHash(done.hash)}`, GIT_STATUS_COLORS['hash']);
				this.insertText(` ${done.subject}\n`, GIT_STATUS_COLORS['subject']);
			}

			const gitLog = await this.git.getLog(onto);
			const hash = gitLog.getHash();
			const subject = gitLog.getSubject();
			const msg = (subject.length > 0) ? subject : '(no commit message)';
			this.insertText(`onto ${hash}`, GIT_STATUS_COLORS['hash']);
			this.insertText(` ${msg}\n`, GIT_STATUS_COLORS['subject']);
		} else if (this.git.rebaseApply()) {
		}
	}
